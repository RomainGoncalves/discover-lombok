...
add_action('widgets_init','ct_v_newsticker_load_widgets_temp');


function ct_v_newsticker_load_widgets_temp(){
		register_widget("ct_v_newsticker_Widget_temp");
}
 ...
 *
 */
class ct_v_newsticker_Widget_temp extends WP_widget{

	/**
	 * Widget setup.
	 */	
	function ct_v_newsticker_Widget_temp(){
		
		/* Widget settings. */	
		$widget_ops = array( 'classname' => 'ct_v_newsticker_widget_temp', 'description' => __( 'News Ticker Vertical' , 'color-theme-framework' ) );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 350, 'id_base' => 'ct_v_newsticker_widget_temp' );
		
		/* Create the widget. */
		$this->WP_Widget( 'ct_v_newsticker_widget_temp', __( 'CT: News Ticker Vertical - TEMP' , 'color-theme-framework' ) ,  $widget_ops, $control_ops );
	...
	<div id="v-newsticker-<?php echo $time_id; ?>">
	  <ul>
		...
		?>
	      <li class="clearfix">
			<div class="news-date" style="background-color: <?php echo $cat_color; ?>">
			  <?php echo esc_attr( get_the_date( 'd.m.y' ) ); ?>
			  <span class="arrow-right" style="border-left: 10px solid <?php echo $cat_color; ?>"></span>
			</div><!-- .news-date -->
			
			<div class="news-title">
			  <h4><a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'color-theme-framework' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php echo strip_tags(substr(the_title('','',false), 0, 22 ) ) . ' ...'; ?></a></h4>
			</div><!-- .title-mask -->
