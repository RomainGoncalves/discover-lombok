...
add_action('widgets_init','CT_one_columng_cat_remove_widgets_temp', 11);


function CT_one_columng_cat_remove_widgets_temp(){
		unregister_widget("CT_one_columng_cat_Widget");
}

//Add custom widget
add_action('widgets_init','CT_one_columng_cat_load_widgets_temp', 11);


function CT_one_columng_cat_load_widgets_temp(){
		register_widget("CT_one_columng_cat_Widget_temp");
}
 ...
 *
 */
class CT_one_columng_cat_Widget_temp extends WP_widget{

	/**
	 * Widget setup.
	 */	
	function CT_one_columng_cat_Widget_temp(){
		
		/* Widget settings. */	
		$widget_ops = array( 'classname' => 'ct_one_columng_cat_widget_temp', 'description' => __( 'One Column Category Widget' , 'color-theme-framework' ) );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 350, 'id_base' => 'ct_one_columng_cat_widget_temp' );
		
		/* Create the widget. */
		$this->WP_Widget( 'ct_one_columng_cat_widget_temp', __( 'CT: One Column Category Widget Temp' , 'color-theme-framework' ) ,  $widget_ops, $control_ops );
	...
	function widget($args,$instance){
	    ...
	    <?php
        	...
        	if (get_locale() == 'fr_FR') {
        		...
        		echo esc_attr( get_the_date( 'd F' ) );
