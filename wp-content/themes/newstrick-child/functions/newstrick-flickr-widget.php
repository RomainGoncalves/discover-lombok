...
//remove theme's widget
add_action('widgets_init', 'CT_remove_flickr_widgets', 11);

function CT_remove_flickr_widgets()
{
	unregister_widget('CT_Flickr_Widget');
}

//Loads ours
// add_action('widgets_init', 'CT_load_flickr_widgets_Temp');

function CT_load_flickr_widgets_Temp()
{
	register_widget('CT_Flickr_Widget_Temp');
}
 ...
 *
 */
class CT_Flickr_Widget_Temp extends WP_Widget {
	...
	function CT_Flickr_Widget() {
		...
		/* Widget settings. */
		$widget_ops = array('classname' => 'ct_flickr_widget', 'description' => __( 'CT: Flickr Widget TEMP', 'color-theme-framework' ) );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 350, 'id_base' => 'ct_flickr_widget' );

		//var_dump($this->WP_Widget()) ;
		/* Create the widget. */		
		$this->WP_Widget( 'ct_flickr_widget12', __( 'CT: Flickr - TEMP' , 'color-theme-framework' ), $widget_ops);
	...
	function widget( $args, $instance ) {
		...
		if ( !is_admin() ) {
			/* Flickr */
			wp_register_script('jquery-flickr',get_stylesheet_directory_uri().'/js/jflickrfeed.min.js',false, null , true);
